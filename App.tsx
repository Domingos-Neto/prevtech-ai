...
const App: React.FC = () => {
  ...
  // Notifica o sistema pai para redimensionar o container (Envia para a Esquerda agora)
  useEffect(() => {
  ...
  return (
    <div className="relative h-screen w-screen bg-transparent overflow-hidden flex flex-col items-start justify-end p-4">
      {/* Botão Flutuante (Bubble) - LADO ESQUERDO */}
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className={`h-14 w-14 rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.3)] flex items-center justify-center transition-all duration-300 z-[9999] group ${
      ...
      {/* Janela de Chat - LADO ESQUERDO */}
      <div className={`absolute bottom-20 left-4 w-[90vw] md:w-[380px] h-[75vh] max-h-[580px] bg-white rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.4)] flex flex-col overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] z-[9998] border border-slate-100 ${
        ...
        <main ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#fcfdfe]">
          ...
          {chatState.error && (
            <div className="p-3 bg-red-50 text-red-600 text-[10px] rounded-xl border border-red-100 text-center">
              {chatState.error}
            </div>
          )}
        </main>
